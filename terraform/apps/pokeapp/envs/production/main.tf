terraform {
  ...
  backend "s3" {
    ...
    key    = "pokeapp-production.tfstate"
...
data "aws_acm_certificate" "prod" {
  ...
  domain   = "pokeapp.systemdesign-apu.com"
}

module "cloudfront" {
  source              = "../../modules/cloudfront"
  zone_name           = "pokeapp.systemdesign-apu.com"
...
module "s3" {
  source         = "../../modules/s3"
  s3_bucket_name = "pokeapp-production"
  cloudfront_arn = module.cloudfront.arn
}

data "aws_route53_zone" "pokeapp" {
  name = "pokeapp.systemdesign-apu.com"
}
...
module "private_subnet" {
  ...
  name_prefix          = "pokeapp-production"
  vpc_id               = data.aws_vpc.pokeapp.id
  cidr_block_1a        = "10.2.131.0/24"
  cidr_block_1c        = "10.2.132.0/24"
...
module "alb_target_group" {
  ...
  zone_name              = "api.pokeapp.systemdesign-apu.com"
  path_pattern           = "/*"
  vpc_id                 = data.aws_vpc.pokeapp.id
  port                   = 3000
  alb_tg_name            = "pokeapp-production"
...
module "cloudwatch" {
  ...
  log_group_name = "pokeapp-production"
}
...
module "ecs" {
  ...
  container_name   = "pokeapp-production"
