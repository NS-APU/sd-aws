resource "aws_ecs_cluster" "pokeapp" {
  name = "${var.name_prefix}-cluster"
}

resource "aws_ecs_task_definition" "pokeapp" {
...
resource "aws_ecs_service" "pokeapp" {
  name            = "${var.name_prefix}-service"
  cluster         = aws_ecs_cluster.pokeapp.id
  task_definition = aws_ecs_task_definition.pokeapp.arn
  launch_type     = "FARGATE"
  desired_count   = 1
