resource "aws_acm_certificate" "pokeapp" {
...
resource "aws_route53_record" "dns_verify" {
  for_each = {
    for dvo in aws_acm_certificate.pokeapp.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "pokeapp" {
  certificate_arn         = aws_acm_certificate.pokeapp.arn
  validation_record_fqdns = [for record in aws_route53_record.dns_verify : record.fqdn]
}

resource "aws_acm_certificate" "pokeapp-stag" {
...
resource "aws_route53_record" "dns_verify-stag" {
  for_each = {
    for dvo in aws_acm_certificate.pokeapp-stag.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "pokeapp-stag" {
  certificate_arn         = aws_acm_certificate.pokeapp-stag.arn
